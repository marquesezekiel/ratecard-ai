/**
 * Brand Message Analyzer - Unified DM + Email Parser
 *
 * Analyzes brand outreach messages from any channel (DMs, emails, etc.)
 * to extract opportunity details, detect gift offers, and provide recommendations.
 *
...
const EMAIL_PATTERNS = {
  headers: [
    /^(Subject|From|To|Date|Cc|Bcc):/im,
    /^On .+ wrote:$/im,
  ],
  greetings: [
    /^Dear\s+[A-Z]/im,
  ...
  signatures: [
    /@[a-z0-9.-]+\.(com|io|co|org|net|edu)/i,
    /\b(Marketing|Partnerships?|Brand|PR|Communications?)\s+(Manager|Director|Lead|Coordinator|Specialist)\b/i,
    /\b(Tel|Phone|Mobile|Fax):\s*[+\d\s()-]+/i,
  ],
};

// DM detection patterns
const DM_PATTERNS = {
  greetings: [
    /^Hey!|^Hi!|^Hii+/i,
    /^Hey babe|^Hey girl|^Hey love/i,
    /^Yo!|^Heyyy/i,
  ],
  language: [
    /DM me|slide into|check your DMs/i,
    /link in bio|swipe up/i,
    /collab|collaboration/i,
  ],
  platforms: {
    instagram_dm: /instagram|@[\w.]+|#\w+/i,
    tiktok_dm: /tiktok|fyp|viral|duet/i,
    twitter_dm: /twitter|tweet|x\.com/i,
    linkedin_dm: /linkedin|connection|network|professional/i,
  },
};
...
const MESSAGE_ANALYZER_SYSTEM_PROMPT = `You are an expert at analyzing brand outreach messages (DMs and emails) to content creators. Extract structured information and assess the opportunity quality.

Return ONLY valid JSON with this exact structure (no markdown, no explanation):
{
  "brandName": "string or null",
  "brandHandle": "string or null",
  "brandEmail": "string or null",
  "brandWebsite": "string or null",
  ...
  "emailMetadata": {
    ...
    "companySignature": "string or null",
...
- Look for email headers (Subject:, From:, To:), formal greetings (Dear X), email closings (Best regards), and company signatures
- DMs tend to be more casual, shorter, and use platform-specific language
...
- Extract subject line if present (from "Subject:" header or inferred)
- Extract sender name and email from "From:" or signature
- Extract company name from signature block
- Note if attachments are mentioned
...
/**
 * Check if the message text contains gift-related phrases.
 */
...
/**
 * Check if the message text shows mass outreach patterns.
...
/**
 * Detect whether the message is a DM or email based on content patterns.
 */
export function detectMessageSource(content: string): { source: MessageSource; confidence: "high" | "medium" | "low" } {
  let emailScore = 0;
  let dmScore = 0;
  const lowerContent = content.toLowerCase();

  // Check email patterns
  for (const pattern of EMAIL_PATTERNS.headers) {
    if (pattern.test(content)) emailScore += 3;
  }
  for (const pattern of EMAIL_PATTERNS.greetings) {
    if (pattern.test(content)) emailScore += 2;
  }
  for (const pattern of EMAIL_PATTERNS.closings) {
    if (pattern.test(content)) emailScore += 2;
  }
  for (const pattern of EMAIL_PATTERNS.signatures) {
    if (pattern.test(content)) emailScore += 2;
  }

  // Check DM patterns
  for (const pattern of DM_PATTERNS.greetings) {
    if (pattern.test(content)) dmScore += 2;
  }
  for (const pattern of DM_PATTERNS.language) {
    if (pattern.test(content)) dmScore += 1;
  }

  // Check platform-specific DM patterns
  let detectedPlatform: MessageSource = "other";
  for (const [platform, pattern] of Object.entries(DM_PATTERNS.platforms)) {
    if (pattern.test(lowerContent)) {
      dmScore += 2;
      detectedPlatform = platform as MessageSource;
    }
  }

  // Determine source and confidence
  const totalScore = emailScore + dmScore;
  let source: MessageSource;
  let confidence: "high" | "medium" | "low";

  if (emailScore > dmScore) {
    source = "email";
    confidence = emailScore >= 5 ? "high" : emailScore >= 3 ? "medium" : "low";
  } else if (dmScore > emailScore) {
    source = detectedPlatform !== "other" ? detectedPlatform : "instagram_dm"; // Default to Instagram
    confidence = dmScore >= 4 ? "high" : dmScore >= 2 ? "medium" : "low";
  } else {
    // Equal or no signals - default based on message length and formality
    if (content.length > 500 || /^Dear|^Hello [A-Z]/i.test(content)) {
      source = "email";
    } else {
      source = "instagram_dm";
    }
    confidence = totalScore > 0 ? "low" : "low";
  }
 ...
 */
function extractEmailMetadata(content: string): EmailMetadata | null {
  ...
  if (fromMatch) {
    ...
    if (emailMatch) {
      metadata.senderEmail = emailMatch[1];
      const nameBeforeEmail = fromLine.split('<')[0].trim();
      if (nameBeforeEmail) {
        metadata.senderName = nameBeforeEmail.replace(/"/g, '');
      }
    } else {
      const simpleEmailMatch = fromLine.match(/([^\s]+@[^\s]+)/);
      if (simpleEmailMatch) {
        metadata.senderEmail = simpleEmailMatch[1];
      }
    }
  }

  // Extract signature block (look for common patterns at end of message)
  const signaturePatterns = [
    /(?:Best regards|Sincerely|Kind regards|Thanks|Cheers),?\s*\n+([A-Z][a-zA-Z\s]+)\n/i,
    /--\s*\n(.+(?:\n.+)*)/,
  ];
  ...
  for (const pattern of signaturePatterns) {
    ...
    if (match) {
      metadata.companySignature = match[1].trim().slice(0, 200); // Limit length
      // Try to extract sender name from signature
      const nameLine = match[1].split('\n')[0].trim();
      if (!metadata.senderName && /^[A-Z][a-z]+ [A-Z][a-z]+/.test(nameLine)) {
        metadata.senderName = nameLine;
      }
      break;
    }
  }
  ...
  metadata.hasAttachments = /attached|attachment|see attached|find attached/i.test(content);

  // Return null if nothing was extracted
  if (Object.keys(metadata).every(k => metadata[k as keyof EmailMetadata] === undefined || metadata[k as keyof EmailMetadata] === false)) {
    return null;
  }
 ...
 */
function generateNextSteps(analysis: Partial<MessageAnalysis>): string[] {
  const steps: string[] = [];
...
/**
 * Validate and normalize LLM output to MessageAnalysis structure.
 */
function validateAndNormalize(
  data: unknown,
  _profile: CreatorProfile,
  detectedSource: MessageSource
): Omit<MessageAnalysis, "recommendedResponse" | "suggestedRate" | "dealQualityEstimate" | "nextSteps"> {
  ...
  let emailMetadata: EmailMetadata | undefined = undefined;
  if (parsed.emailMetadata && typeof parsed.emailMetadata === "object") {
    const em = parsed.emailMetadata as Record<string, unknown>;
    emailMetadata = {
      subject: em.subject ? String(em.subject) : undefined,
      senderName: em.senderName ? String(em.senderName) : undefined,
      senderEmail: em.senderEmail ? String(em.senderEmail) : undefined,
      companySignature: em.companySignature ? String(em.companySignature) : undefined,
      hasAttachments: Boolean(em.hasAttachments),
    };
  }
  ...
  return {
    // Source detection
    detectedSource,
    sourceConfidence: "high", // Will be updated by caller

    // Brand identification
    brandName: parsed.brandName ? String(parsed.brandName) : null,
    brandHandle: parsed.brandHandle ? String(parsed.brandHandle) : null,
    brandEmail: parsed.brandEmail ? String(parsed.brandEmail) : null,
    brandWebsite: parsed.brandWebsite ? String(parsed.brandWebsite) : null,
...
async function parseWithGroq(
  ...
  const response = await client.chat.completions.create({
    model: GROQ_MODEL,
    messages: [
      { role: "system", content: MESSAGE_ANALYZER_SYSTEM_PROMPT },
      { role: "user", content: `Analyze this brand message:\n\n${messageText}` },
    ],
    temperature: 0.1,
...
async function parseWithGemini(
  ...
  const prompt = `${MESSAGE_ANALYZER_SYSTEM_PROMPT}\n\nAnalyze this brand message:\n\n${messageText}`;
...
/**
 * Attempt to parse a message with retry logic.
 */
async function parseWithRetry(
  ...
  for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
    try {
      return await parser(messageText);
    } catch (error) {
...
/**
 * Analyze a brand message (DM or email) and return structured analysis.
 *
 * Uses Groq as primary provider, falls back to Gemini on error.
 * Auto-detects whether the message is a DM or email.
 * Detects gift offers, analyzes tone/quality, and generates recommended responses.
 *
 * @param input - The message input containing content and optional hints
 * @param creatorProfile - The creator's profile for rate calculations
 * @returns Complete message analysis with recommendations
...
export async function analyzeMessage(
  ...
  const { content, sourceHint } = input;

  // Validate input
  if (!content || content.trim().length < MIN_MESSAGE_LENGTH) {
    throw new Error(
  ...
  const detection = detectMessageSource(trimmedContent);
  const detectedSource = sourceHint || detection.source;
  const sourceConfidence = sourceHint ? "high" : detection.confidence;

  // Extract email metadata if it's an email
  const localEmailMetadata = detectedSource === "email" ? extractEmailMetadata(trimmedContent) : null;

  // Try Groq first
  let llmResult: Record<string, unknown>;
  try {
    llmResult = await parseWithRetry(trimmedContent, parseWithGroq, "Groq");
  } catch (groqError) {
    console.warn("Groq failed, falling back to Gemini:", groqError);

    // Fall back to Gemini
    try {
      llmResult = await parseWithRetry(trimmedContent, parseWithGemini, "Gemini");
    } catch (geminiError) {
      console.error("Both Groq and Gemini failed:", geminiError);
      throw new Error(
        "Failed to analyze message with all available providers. Please try again later."
      );
    }
  }

  // Validate and normalize the LLM output
  const baseAnalysis = validateAndNormalize(llmResult, creatorProfile, detectedSource);

  // Merge local email metadata with LLM-extracted metadata
  if (localEmailMetadata && detectedSource === "email") {
    baseAnalysis.emailMetadata = {
      ...localEmailMetadata,
      ...baseAnalysis.emailMetadata,
    };
  }
  ...
  baseAnalysis.sourceConfidence = sourceConfidence;
...
/**
 * Quick check if a message appears to be a gift offer.
 ...
 */
export function isLikelyGiftOffer(text: string): boolean {
  return containsGiftIndicators(text);
}

/**
 * Quick check if a message appears to be mass outreach.
 * Can be used for fast pre-filtering without full LLM analysis.
 *
 * @param text - The message text to check
 * @returns True if the text contains mass outreach signals
 */
export function isLikelyMassOutreach(text: string): boolean {
  return containsMassOutreachSignals(text);
}

/**
 * Backwards compatibility alias for parseDMText.
 * @deprecated Use analyzeMessage instead
 */
export async function parseDMText(
  dmText: string,
  creatorProfile: CreatorProfile
): Promise<MessageAnalysis> {
  return analyzeMessage({ content: dmText }, creatorProfile);
}
